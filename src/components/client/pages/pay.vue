        ...
        <table class="table">
          ...
          <tfoot>
            <tr>
              ...
              <td class="lead"><b>{{ order.total }}</b></td>
            </tr>
          </tfoot>
        </table>
        <hr>
...
import Alert from '../../AlertMsg'
  export default {
    ...
    methods: {
      getPaymentInfo() {
        const api = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/order/${this.order_id}`;
        this.$http.get(api).then((response) => {
          this.order = response.data.order;
          if(response.data.order.is_paid) {
            this.$bus.$emit('messsage:push', '付款已完成', 'success')
          }
        });
      },
      payOrder() {
        this.isLoading = true;
        const api = `${process.env.APIPATH}/api/${process.env.CUSTOMPATH}/pay/${this.order_id}`
